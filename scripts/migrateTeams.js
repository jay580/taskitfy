...
const mapTeamName = (teamName) => {
    if (!teamName)
        return null;
    const normalized = teamName.trim().toLowerCase();
    if (validTeams.includes(normalized))
        return normalized;
    // Custom mapping rules (if any exist)
    if (normalized === 'team earth')
        return 'earth';
    if (normalized === 'team water')
        return 'water';
    if (normalized === 'team fire')
        return 'fire';
    if (normalized === 'team wind')
        return 'wind';
...
async function run() {
    ...
    for (const teamId of validTeams) {
        const teamRef = (0, firestore_1.doc)(db, 'teams', teamId);
        const teamDoc = await (0, firestore_1.getDocs)((0, firestore_1.collection)(db, 'teams'));
        let exists = false;
        teamDoc.forEach(d => { if (d.id === teamId)
            exists = true; });
        if (!exists) {
            await (0, firestore_1.setDoc)(teamRef, {
                id: teamId,
    ...
    for (const m of validMappings) {
        const userRef = (0, firestore_1.doc)(db, 'users', m.uid);
